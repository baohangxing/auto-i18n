...
4. vue 模版解析过程中如果遇到元素的属性值是 `''`, 例如 `<Comp propName=''/>`, 转换后会变成 `<Comp propName/>`, 这个问题出现的原因是这两种情况在解析之后都是得如下的解析结果：
...
5. vue 模版解析过程中如果只是在 <template> 中出现了需要替换的中文，在 <script> 中没有出现需要替换的中文，配置的 `I18nCallRule.importDeclaration` 和 `I18nCallRule.variableDeclaration` 不会被引入，您可能需要手动添加引用和声明
